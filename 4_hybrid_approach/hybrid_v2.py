...
OUT_DIR = os.path.join(BASE_DIR, "results/hybrid_unet_rowmask_v2_soft")
...
# Hybrid rules (softer/less restrictive version)
TH_WEED_BASE = 0.50          # base threshold for weed
TH_WEED_IN_ROW = 0.60        # inside crop row, require somewhat higher confidence (softer than v1)
MIN_SIZE_IN_ROW = 2          # softer cleanup inside crop row
...
class HybridPatchDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # remap reference mask to 0 crop / 1 weed / 2 soil / 255 ignore
...
def remove_small_components(binary_mask, min_size=4):
...
def apply_hybrid_rules(prob_weed, pred_base, rowmask):
    """
    prob_weed: (H,W) predicted probability for the weed class
    pred_base: (H,W) base predicted classes 0 crop / 1 weed / 2 soil
    rowmask:   (H,W) 1 = inside crop row / 0 = outside
    """

    pred_h = pred_base.copy()

    # 1) Generate weed candidates from probability
    cand_base = prob_weed >= TH_WEED_BASE
    cand_row = prob_weed >= TH_WEED_IN_ROW

    # 2) Inside crop row: stricter criterion
    weed_final = np.zeros_like(prob_weed, dtype=np.uint8)
    weed_final[rowmask == 0] = cand_base[rowmask == 0]
    weed_final[rowmask == 1] = cand_row[rowmask == 1]

    # 3) Only clean up small objects inside crop row
    weed_in_row = ((weed_final == 1) & (rowmask == 1)).astype(np.uint8)
    weed_out_row = ((weed_final == 1) & (rowmask == 0)).astype(np.uint8)

    weed_in_row = remove_small_components(weed_in_row, min_size=MIN_SIZE_IN_ROW)

    weed_final = ((weed_in_row == 1) | (weed_out_row == 1)).astype(np.uint8)

    # 4) Build final prediction:
    # where weed_final = 1 => weed class
    # elsewhere, keep base prediction, except pixels that were weed but no
    # longer qualify, which become crop
    pred_h[pred_h == 1] = 0
    pred_h[weed_final == 1] = 1
...
with torch.no_grad():
    for imgs, masks, rowmasks, names in loader:
        ...
        for i in range(len(names)):
            ...
            pred_h = apply_hybrid_rules(pm, pred_b, rm)
...
with open(summary_path, "w", encoding="utf-8") as f:
    ...
    f.write(f"TH_WEED_BASE: {TH_WEED_BASE}\n")
    f.write(f"TH_WEED_IN_ROW: {TH_WEED_IN_ROW}\n")
