"""
Train a DeepLabv3 (ResNet-50 backbone) model for weed segmentation
(crop / weed / soil classes) from multiband patches, adapting the first
convolutional layer to accept more than 3 input channels.

Requires: torch, torchvision, rasterio, numpy
"""
...
OUT_DIR = os.path.join(BASE_DIR, "results/deeplabv3_rgb_nir_re_indices_30ep")
...
class PatchDataset(Dataset):
    def __init__(self, img_dir, mask_dir):
        ...
        assert len(self.img_paths) > 0, f"No patches found in:\n{img_dir}"
    ...
    def __getitem__(self, idx):
        ...
        # Per-patch, per-band normalization
...
# Replace the first conv layer to accept `in_channels` input bands
...
with open(summary_path, "w", encoding="utf-8") as f:
    ...
    f.write(f"Best F1 weed: {best_test_f1_weed:.6f}\n\n")
...
print("\nTraining finished.")
print(f"Best F1 weed: {best_test_f1_weed:.4f}")
